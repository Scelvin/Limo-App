...
@section('content')
    ...
    <div class="col-6 py-3 bg-light border p-5">
        <form action="{{ route('clients.store') }}" method="PUT" id="albumForm" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="form group py-2">
                <label for="total-enquiry">Total Enquiries: {{ $total_enquiry->count() }}</label>
            </div>

            <div class="form group py-2">
                <label for="last-enquiry">Last enquiry: {{ $last_enquiry }}</label>
            </div>
